...
export class VaultService {
  ...
  unlock(workspace: string, password: string): { unlocked: boolean; retryAfterMs?: number; corrupted?: boolean } {
    ...
    try {
      if (this.store.exists()) {
        ...
        const envelopeData = this.store.read()
        const envelope = this.store.deserialize(envelopeData)
  ...
  private persist(): void {
    ...
    const serialized = this.store.serialize(envelope)
    this.store.write(serialized)
  }
}

export function createVaultService(store: VaultStore): VaultService {
  return new VaultService(store)
}
