...
/** Default key — apps must call configureThemeStorageKey before boot. */
let themeStorageKey = 'theme'

/** Inject per-app localStorage key (e.g. engrenacode:theme). */
export function configureThemeStorageKey(key: string): void {
  if (key.trim() === '') {
    throw new Error('theme storage key must be non-empty')
  }
  themeStorageKey = key
}
