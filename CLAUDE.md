...
- `Smoke · Access level · Sempre usar "Auto-accept edits" (nunca "Supervised") ao disparar turno real via automação porque Supervised = --permission-mode default exige aprovação interativa via stdin que não existe em spawn headless, e todo Edit falha com "Need permission"`
- `Docs · Ondas · Sempre manter a tabela "Ondas (PRD §8)" de docs/PROGRESS.md espelhando toda feature da tabela de dependências do PRD (pendentes inclusas) com paralelismo explícito por linha, porque backlog descrito só em texto narrativo ("Próxima frente de produto") sai do radar e ondas com pendência ficam marcadas "Completa"`
...
- `Skills · Manutenção · Nunca gravar dado volátil (n testes verdes, "os 12 handlers", data de passagem) em .claude/skills/**; padrão durável e ID do achado ficam na skill, número e evidência só em docs/AUDIT-CODE-REVIEW.md, porque snapshot em skill envelhece e a próxima sessão age sobre fato falso`
- `Skills · Estrutura · Sempre pôr material auxiliar de skill em <skill>/references/*.md (padrão de playwright-cli/spec-writer) e manter SKILL.md como roteamento, porque frontmatter+corpo entram no contexto sempre e o resto só sob demanda`
