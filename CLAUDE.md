...
- `Homologação · Onde mora · Sempre carregar a skill `homologar` antes de smoke ao vivo, roteiro de runbook ou qualquer marcação de critério que descreva o que o usuário vê. Ela é dona do método (provar pelo negativo, estado terminal tem de ser usável, ler evidência em vez de instrumentar, fixture marcada e removida, não editar no meio da bateria) e dos bindings deste repo (portas, unlock por API, caminho do banco, armadilhas de sandbox/WebSocket/kill). As regras `Smoke · *` e `Runner · Processos` que ficavam aqui foram para lá em 2026-08-20 — mesma substância, um dono só`
...
- `Docs · Índice · Sempre listar em apps/engrena-code/docs/README.md todo arquivo e pasta da raiz de docs/ (F* novas, archived/, _shared/, smoke agregado) e nunca gravar status de feature no FAQ do índice porque o split de 2026-08-20 deixou F32–F35 órfãos e o FAQ lia F31 como pendente depois da entrega; status vive em PROGRESS.md`
