...
- `Workspace · Fila · Sempre passar queueLength para routeComposerSend e enfileirar quando a fila não está vazia, mesmo com a thread assentada, porque a rota decidia só por thread.state: mensagem nova virava send_follow_up e furava a fila. O gatilho é o Parar — cancelled fica fora de TURN_RECONCILED_STATES de propósito, então a fila não drena sozinha; ela precisa do CTA "Executar agora" (surface.queuePaused) e de rótulo próprio na bolha, senão fica dizendo "aguarde o turno atual terminar" sem turno nenhum`
