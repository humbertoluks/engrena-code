...
**8. Fechamento visual (pós-implementação)** - `ui.md`/`copy.md` escritos catalogando a anatomia/copy já implementadas em `TerminalDock.tsx`/`TerminalPane.tsx`. Gap identificado contra o PRD Seção 6: aviso de "sem sandbox adicional do EngrenaCode" ainda não existe em nenhuma string da UI — pendente de decisão de design (`ui.md` Perguntas em aberto), não bloqueia os 4 critérios de aceitação funcionais (já `[x]` em `docs/PRD.md`).
