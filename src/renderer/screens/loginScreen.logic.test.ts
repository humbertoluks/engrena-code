...
import { classifyUnlockFailure, messageForError } from './loginScreen.logic'

describe('loginScreen.logic', () => {
  describe('messageForError', () => {
    it('returns copy for invalid, corrupted and network', () => {
      expect(messageForError('invalid', 0)).toBe('Workspace ou senha inválidos.')
      expect(messageForError('corrupted', 0)).toContain('danificado')
      expect(messageForError('network', 0)).toContain('servidor local')
    })

    it('formats backoff with ceil seconds', () => {
      expect(messageForError('backoff', 1500)).toBe('Muitas tentativas. Tente novamente em 2s.')
      expect(messageForError('backoff', 0)).toBe('Muitas tentativas. Tente novamente em 0s.')
    })

    it('returns null when kind is null', () => {
      expect(messageForError(null, 0)).toBeNull()
    })
  })
  ...
  describe('classifyUnlockFailure', () => {
    it('classifies vault_corrupted and 422 as corrupted', () => {
      expect(
        classifyUnlockFailure({ status: 400 }, { unlocked: false, error: { code: 'vault_corrupted' } })
    ...
    it('defaults remaining failures to invalid', () => {
      expect(
        classifyUnlockFailure({ status: 401 }, { unlocked: false, error: { code: 'invalid_password' } })
