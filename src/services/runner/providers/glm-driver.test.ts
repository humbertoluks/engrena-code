...
import { resetFetchForTesting, runHttpTurn, setFetchForTesting, testConnection } from './glm-driver'
...
function baseInput(overrides: Partial<ProviderTurnInput> = {}): ProviderTurnInput {
  return {
    ...
    apiKey: 'abcdef01234.5678secretpart',
    onEvent: () => {},
    ...overrides,
  }
}
...
describe('runHttpTurn', () => {
  ...
  it('throws image_not_supported when images are present (GLM is text-only)', async () => {
    await expect(
  ...
  it('sends the Authorization header and resolves the response text', async () => {
    ...
    expect(capturedHeaders.Authorization).toBe('Bearer abcdef01234.5678secretpart')
  })
...
describe('testConnection', () => {
  ...
  it('returns success:true on 2xx', async () => {
    ...
    const result = await testConnection('abcdef01234.5678secretpart')
  ...
  it('returns a distinct message for provider_auth_error vs provider_network_error', async () => {
    setFetchForTesting(async () => new Response('nope', { status: 401, statusText: 'Unauthorized' }))
    const authResult = await testConnection('abcdef01234.5678secretpart')

    setFetchForTesting(async () => {
      throw new Error('ECONNREFUSED')
    })
    const networkResult = await testConnection('abcdef01234.5678secretpart')
